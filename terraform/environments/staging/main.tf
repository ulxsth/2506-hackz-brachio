...
output "supabase_url" {
  ...
  value       = module.type2live_staging.supabase_url
  sensitive   = false
}

output "supabase_anon_key" {
  description = "Supabase 匿名キー"
  value       = module.type2live_staging.supabase_anon_key
  sensitive   = true
}

output "vercel_url" {
  description = "Vercel プロジェクトURL"
  value       = module.type2live_staging.vercel_url
  sensitive   = false
}

output "vercel_deployment_url" {
  description = "Vercel デプロイメントURL"
  value       = module.type2live_staging.vercel_deployment_url
  sensitive   = false
}
