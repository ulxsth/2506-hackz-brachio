...
#### システムの概要
指定されたアルファベット一文字を含むIT用語をタイピングして得点を競う、リアルタイムマルチプレイヤーゲーム
...
- **入力方式**: キーボード入力
- **入力検証**: 
  - 通常ターン：提示された単語との完全一致判定
  - 制約ターン：辞書との照合による正誤判定 + 制約条件チェック
- **リアルタイム表示**: 入力中の文字をリアルタイム表示

##### 得点システム
- **得点計算式**: 
  - 通常ターン：`獲得得点 = 単語文字数 × コンボ数`
  - 制約ターン：`獲得得点 = 単語文字数 × 難易度(1-4) × 制約係数 × コンボ数`
- **得点表示**: リアルタイムでの得点更新表示
- **順位表示**: プレイヤー順位のリアルタイム表示

##### ターンシステム
- **ターンタイプ**: 2種類のターンをランダムに出題
  1. **通常タイピングターン（約83%）**: 指定されたIT用語をタイピングする
     - 画面にIT用語を表示し、プレイヤーがその通りにタイピング
     - 正確性と速度を重視したタイピング練習
  2. **制約ターン（約17%）**: 制約条件に沿った単語を考えて入力する
     - 指定文字を含むIT用語を考えて入力（例：「r」→「react」「server」「jar」）
     - 知識と思考力を重視した問題解決

##### 制約システム（制約ターン用）
- **制約タイプ**: 「指定文字を含む」のみ
- **制約表示**: 現在の指定文字制約条件を画面上に明示
- **制約変更**: パス機能による新しい文字の生成（クールダウン10秒）
...
##### 制約データ管理
- **制約パターン**: 指定文字制約のシンプル設計
- **制約係数**: 文字の出現頻度による動的係数（2-8）
- **動的難易度調整**: 文字の希少性による係数変動
